...
```plaintext
=OFFSET(C1, MATCH("Target", B1:B100, 0) - 1, 0)
```
...
| **INDEX / MATCH (0)** | Linear Search | Optimized in Microsoft 365 via caching | $\mathcal{O}(n)$ |
...
* **Difficulty:** Intermediate.
* **Best Practice:** Load raw data connection-only into the Data Model, and use Power Query to merge/join tables rather than using nested `VLOOKUP` or `XLOOKUP` formulas.
